...
9. Confirm transition into combat: `gameStatus === 'combat'` and `pokerIsActive === true`. The cards UI mounts (mulligan prompt and universal 60s poker decision window). No console errors.
10. Play through combat phases (mulligan -> First Blood -> betting rounds) until one side's HP reaches 0 or chess resumes and chess reaches checkmate, draw, or decisive-material game-over.
