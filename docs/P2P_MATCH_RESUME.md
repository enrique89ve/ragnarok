...
- Any cards/chess/poker state-hash mismatch or transition receipt disagreement sets a session integrity quarantine. The UI shows the pause and every gameplay sender/receiver drops further actions; reconnect preserves the quarantine until the player leaves the match.
