...
const CardPlacementEffect: React.FC<CardPlacementEffectProps> = ({
  ...
  useEffect(() => {
    ...
    const numParticles = particleCount || (isMythic ? 100 : isPremium ? 70 : 40);

    // Color palette for different card types
    const regularColors = ['#fff8e0', '#ffe060', '#ffb900', '#ffd700'];
    const mythicColors = ['#fffacd', '#ffd700', '#daa520', '#ffb90f', '#ffdf00'];
    ...
    const animate = (time: number) => {
      ...
      const duration = isMythic ? 1500 : isPremium ? 1300 : 1000;
  ...
  }, [x, y, isMythic, isPremium, particleCount, particleColors, onComplete]);
