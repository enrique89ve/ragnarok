...
export type PokerViewportCssVarName =
  | `--poker-zone-${PokerViewportZoneId}-${"x" | "y" | "w" | "h" | "rot"}`
...
export type PokerViewportLayoutStyle = CSSProperties &
  Partial<Record<PokerViewportCssVarName, string>>;
...
const HOURGLASS_BASE_Y = 122;
const HOURGLASS_HEIGHT = 128;
const HOURGLASS_RISE_RATIO = 0.25;
const HOURGLASS_RISE = Math.round(HOURGLASS_HEIGHT * HOURGLASS_RISE_RATIO);
const HOURGLASS_Y = HOURGLASS_BASE_Y - HOURGLASS_RISE;
const PLAYER_HERO_WIDTH = 208;
const PLAYER_HERO_HEIGHT = 288;
const BETTING_CONTROLS_HEIGHT = 176;
const BETTING_CONTROLS_DROP_RATIO = 0.32;
const BETTING_CONTROLS_DROP = Math.round(
  BETTING_CONTROLS_HEIGHT * BETTING_CONTROLS_DROP_RATIO,
);
const BATTLE_LOG_HEIGHT = 232;
const BOTTOM_RAIL_GAP = 52;
const HUD_BADGE_HEIGHT = 52;
const CANVAS_W = 1920;
const EDGE = 32;
...
const PLAYER_HAND_CARD_RISE_RATIO = 0.2 + 0.3;
const PLAYER_HAND_CARD_RISE = Math.round(
  PLAYER_HAND_HEIGHT * PLAYER_HAND_CARD_RISE_RATIO,
);
const HERO_X = EDGE;
...
const FIELD_H = 184;
const OPP_FIELD_BASE_Y = 280;
const PLAYER_FIELD_BASE_Y = 520;
const BATTLEFIELD_RISE_RATIO = 0.1;
const BATTLEFIELD_RISE = Math.round(
  (PLAYER_FIELD_BASE_Y + FIELD_H - (OPP_FIELD_BASE_Y - 24)) * BATTLEFIELD_RISE_RATIO,
);
const OPP_FIELD_Y = OPP_FIELD_BASE_Y - BATTLEFIELD_RISE;
const PLAYER_FIELD_Y = PLAYER_FIELD_BASE_Y - BATTLEFIELD_RISE;
...
export function buildPokerViewportLayoutStyle(
  layout: PokerViewportLayout = POKER_VIEWPORT_LAYOUT,
): PokerViewportLayoutStyle {
  const style: PokerViewportLayoutStyle = {
    ...
    "--poker-bottom-rail-y": toPixels(
      layout.zones.bettingControls.y + layout.zones.bettingControls.height,
    ),
