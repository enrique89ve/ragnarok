 ...
 *
 * Anti-Corruption Layer (ACL) adapters translate external vocabularies into
 * the canon. Adding a new external vocabulary = one entry in ADAPTERS.
...
/**
 * 5-tier vocabulary used by `shared/schema.ts` (DB) and `cardJson/schema.ts`.
 *  - 'basic'  → 'common'    (basic is a distribution marker, not a rarity tier)
 *  - 'mythic' → 'legendary' (top tier mapped to lore-aligned 'legendary')
 */
